...
3. **공식 FQDN만 등록:** `www.____________`, `admin.____________` 등 **운영 확정 도메인**에 대해서만 아래 §3~§6 표를 채워 IT에 요청합니다.
...
| OAuth Client ID 전략 | **단일 Client ID** — 운영 FQDN + 로컬 dev origin |
| 동의 화면 — 앱명·로고 | ____________ |
| Authorized JavaScript origins | FO: `https://www.____________` · BO: `https://admin.____________` *(필요 시)* · 로컬: `http://localhost:8080` |
