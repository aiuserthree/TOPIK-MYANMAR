...
- **화면 디자인 handoff (authoritative):** `html/C안/BO(admin)/project/` — `admin-login.html` + `admin.html`(React 18 CDN SPA, 패널 14개: 접수·사진심사·회차·시험장·공지·FAQ·환불·문의·회원·약관·관리자·권한·처리이력). 현재 **mock 데이터** 프로토타입(API 미연동).
- **API 연동 stub:** `html/C안/BO/` — HTML 없음, JS assets 4개만 (`bo-api-data.js`, `panels/notices.js` 등). `html/shared/bo-api-client.js`·`bo-common.js`는 **저장소에 없음**.
- BO는 FO와 **별도 호스팅** (`build.py`는 FO만 `public/`로 복사). IwinV에서는 handoff를 `/admin/` 정적 제공 ([`IWINV_SETUP.md`](IWINV_SETUP.md) §1.11). `python3 build-bo.py`는 **`html/C안/BO/`(stub)만** `public-bo/`로 복사하므로 handoff UI가 포함되지 않음 — 스크립트 수정 또는 symlink 배포 전까지 **BO 정적 배포는 사실상 동작하지 않음**.
