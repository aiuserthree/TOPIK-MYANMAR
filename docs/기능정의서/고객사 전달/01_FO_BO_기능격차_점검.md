...
| 구분 | 스펙 대비 | 비고 |
| --- | --- | --- |
| **FO** | 핵심 플로우(가입·접수·마이페이지·게시판·공지) **구현** | 운영 SMTP·**편의지원 FO UI 보류**·i18n **육안 검수** 권장 |
...
| 5 | i18n **전 페이지** (정적·동적·API 오류) | **2026-06-11 완료** — `topik-i18n-content.js`, `TPKMBt`, `fo_messages`/`fo_api_error`. 고객사·QA **육안 검수** 권장 |
| 6 | 공지 **카테고리 라벨** 다국어 | API `notice_category_label(category, lang)` **MY/EN 반영** + FO `parseError`·`err.*` 키 연동 |
