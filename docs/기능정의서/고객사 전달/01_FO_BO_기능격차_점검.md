...
| 구분 | 스펙 대비 | 비고 |
| --- | --- | --- |
| **FO** | 핵심 플로우(가입·접수·마이페이지·게시판·공지) **구현** | 운영 SMTP·편의지원 UI·i18n 잔여 구간 |
...
| 06 계정 | **이메일 인증·비번 재설정** API·FO 연동 | `auth.py`, `signup.html`, `password-reset.html` |
...
| # | 기능정의서 요구 | 현재 상태 |
| --- | --- | --- |
| 3 | **편의지원(장애인)** 접수 UI | API·채번 로직(`accommodation_requested`) 있음, `register.html` **입력 UI 없음** |
...
| 6 | 공지 **카테고리 라벨** 다국어 | API `notice_category_label()` **KO 단일** 반환 |
...
| 2 | `register.html` **편의지원** 체크 UI | FO | API 준비됨 |
