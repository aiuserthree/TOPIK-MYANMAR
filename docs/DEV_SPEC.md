...
현재 저장소는 **재개발 스캐폴드 단계**입니다. 기존 정적 HTML FO/BO와 Node.js Fastify API(`api/`)는 보존·참조용으로 두고, 신규 화면·API는 `apps/web`(Vite + React)과 `apps/api`(FastAPI)에서 단계적으로 이전합니다. 운영 목표 환경은 IwinV VPS 2대(Web + DB)이며 Docker 없이 nginx + systemd로 배포합니다.
...
| 레거시 Frontend | HTML + CSS + JavaScript | FO: `html/C안/FO` (React 없음); BO UI handoff: `html/C안/BO(admin)/project/` (React 18 CDN 프로토타입); BO API stub: `html/C안/BO` |
...
│   ├── C안/
│   │   ├── FO/              # 레거시 FO 정적 HTML
│   │   ├── BO/              # 레거시 BO API 연동 stub (assets만, HTML 없음)
│   │   └── BO(admin)/       # BO 화면 디자인 handoff (실제 UI·기능정의)
│   │       └── project/     # admin-login.html, admin.html, panels/*.jsx 등
│   └── shared/              # FO 공통 JS (api-client.js만; bo-api-client.js 없음)
├── db/
│   └── migrations/          # PostgreSQL SQL migration
├── packages/
│   └── shared/              # 공통 상수 placeholder (@topik-myanmar/shared)
├── docs/                    # 운영·기능 문서
├── build.py                 # FO → public/ (Vercel 정적 배포용)
├── build-bo.py                # BO → public-bo/ (Vercel 정적 배포용)
├── docker-compose.prod.yml  # 레거시 API 셀프호스트용 (선택)
...
- `api/Dockerfile`, `docker-compose.prod.yml` — 레거시 셀프호스트 배포용(전체 소스 필요)
...
**BO 디자인 handoff** (`html/C안/BO(admin)/project/`) — 실제 BO 화면·IA 기준 프로토타입:
...
React 18 + Babel CDN 기반 **목(mock) 데이터 프로토타입**이며, 운영 API 연동 전 단계입니다. 신규 BO는 최종적으로 `apps/web`으로 이전할 예정입니다.

**BO API stub** (`html/C안/BO/`) — `build-bo.py` 입력 경로. HTML 페이지 없음, JS assets 4개만 존재:

- `assets/bo-api-data.js`
- `assets/panels/notices.js`, `faq.js`, `photos.js`

**공통** (`html/shared/`):

- `api-client.js` — FO API 클라이언트

README·DEPLOY.md에서 언급하는 `bo-api-client.js`, `bo-common.js`, `html/C안/BO/` 아래 BO HTML(`login.html`, `applications.html` 등)은 **저장소에 없습니다.** 실제 BO 화면은 `html/C안/BO(admin)/project/`를 참고하세요.
...
`apps/api/alembic/` 설정은 준비되어 있으나 revision은 없습니다. 초기 단계에서는 SQL migration(`db/migrations/`)을 기준으로 하고, ORM 기반 변경 시 Alembic revision을 추가하는 방식을 권장합니다.
...
**25개** HTML 페이지 존재 (목록·IA: [`html/C안/FO/docs/00_IA.md`](../html/C안/FO/docs/00_IA.md), 상세: [`PROJECT_REVIEW.md`](PROJECT_REVIEW.md) §A). `html/shared/api-client.js`로 API 호출. `build.py`로 Vercel 배포 가능.

### 11.3 레거시 BO

| 경로 | 상태 |
| --- | --- |
| `html/C안/BO(admin)/project/` | **디자인 handoff** — `admin-login.html`, `admin.html`, 패널 14개(`.jsx`), React 18 CDN + mock data. 화면·IA 기준 참조용 |
| `html/C안/BO/` | **API stub** — JS assets 4개만, HTML 없음. `build-bo.py` 입력 경로 |

`html/shared/`에는 `bo-api-client.js`가 없고 `api-client.js`(FO용)만 있습니다. 운영 BO는 `apps/web` 이전 전까지 IwinV에서 handoff 정적 제공 또는 임시 nginx `/admin/` 경로를 사용합니다 ([`IWINV_SETUP.md`](IWINV_SETUP.md) §1.11).
...
- Frontend: `npm run build` → nginx가 `dist/` 제공
...
`api/Dockerfile`·`docker-compose.prod.yml`은 레거시 Fastify API 전체 소스가 필요합니다.
...
- [`docs/DEPLOY.md`](DEPLOY.md) — Vercel/Railway·셀프호스트 체크리스트
...
| 레거시 FO 정적 | http://localhost:8080 (예시, Live Server) |
...
| 경로 | 처리 |
| --- | --- |
| `https://YOUR_DOMAIN/` | `apps/web/dist/` (SPA) |
