...
if (config.appEnv === "production" && storageMode() === "local") {
    app.log.warn("STORAGE_PROVIDER=local in production — uploads are stored on ephemeral disk and are lost on redeploy. " +
...
const enablePasswordExpiryCron = String(process.env.ENABLE_PASSWORD_EXPIRY_CRON ?? "").toLowerCase() === "true";
if (enablePasswordExpiryCron) {
    ...
    const runBatch = () => {
        void runPasswordExpiryBatch(pool)
            .then((n) => {
            if (n > 0)
                app.log.info({ queued: n }, "password_expiry batch");
        })
