...
const { DEFAULT_THEME, FOOTER, THEME, TRANSACTIONAL } = rawData;
const BY_TEMPLATE_KEY = Object.fromEntries(TRANSACTIONAL.filter((t) => t.templateKey).map((t) => [t.templateKey, t]));
function normalizeLocale(locale) {
    const loc = (locale ?? "ko").toLowerCase();
    return loc === "my" || loc === "en" ? loc : "ko";
}
function resolveTemplate(tpl, locale) {
...
function blockVisible(b, vars) {
    if (!b.showWhen)
        return true;
    return Object.keys(b.showWhen).every((k) => vars[k] != null && String(vars[k]) === String(b.showWhen[k]));
}
function esc(s) {
...
function ctaBlock(t, tpl, vars) {
    if (!tpl.ctas?.length)
        return "";
    const rows = tpl.ctas
        .map((c, i) => `<tr><td style="padding-top:${i === 0 ? 0 : 10}px;">${button(t, c, vars)}</td></tr>`)
        .join("");
...
function infoTable(t, b, vars) {
    const rows = (b.rows ?? [])
        .map((r, i) => `<tr class="ed-info-row">
...
function renderHtml(theme, tpl, locale, vars) {
...
export function renderEmail(templateKey, locale, variables = {}) {
...
export const TEMPLATE_KEYS = Object.keys(BY_TEMPLATE_KEY);
export function templateSupportsLocale(templateKey, locale) {
    const tpl = BY_TEMPLATE_KEY[templateKey];
    if (!tpl?.i18n)
