...
import { buildEmailDefaults, enqueueEmail, formatVerificationCode, } from "../lib/email-templates/enqueue-notification.js";
import { signAuthTokens as signTokens } from "../lib/auth.js";
import { savePhoto, StorageError } from "../lib/storage.js";
import { genderToCode, isUnderMinimumAge, isValidEmail, isValidPassword, MIN_SIGNUP_AGE_YEARS, normalizeBirthDate, } from "../lib/validation.js";
function signVerificationToken(email) {
    return jwt.sign({ typ: "email_verify", email }, config.jwtSecret, { expiresIn: "30m" });
}
function verifyVerificationToken(token) {
    try {
        const decoded = jwt.verify(token, config.jwtSecret);
        if (decoded.typ !== "email_verify" || !decoded.email)
            return null;
...
export async function authSignupRoutes(app) {
    app.post("/api/v1/auth/send-verification-code", { config: { rateLimit: { max: 5, timeWindow: "1 minute" } } }, async (req, reply) => {
        ...
        try {
            ...
            const recent = await pool.query(`SELECT id FROM email_verification_codes
           ...
           ORDER BY id DESC LIMIT 1`, [email]);
            ...
            await pool.query(`INSERT INTO email_verification_codes (email, code, expires_at)
    ...
    app.post("/api/v1/auth/verify-email", async (req, reply) => {
        ...
        try {
            const { rows } = await pool.query(`SELECT id, code, expires_at, consumed_at
           ...
           LIMIT 5`, [email]);
            ...
            await pool.query(`UPDATE email_verification_codes SET consumed_at = NOW() WHERE id = $1`, [match.id]);
            return {
                verified: true,
                email,
    ...
    app.post("/api/v1/auth/register", { config: { rateLimit: { max: 5, timeWindow: "1 minute" } } }, async (req, reply) => {
        ...
        try {
         ...
         RETURNING id, email, name_ko, name_en`, [
            ...
            if (photoFileId) {
                await client.query(`UPDATE file_attachments SET owner_id = $1 WHERE id = $2`, [userId, photoFileId]);
            }
            const termIds = Array.isArray(body.terms_agreed)
           ...
           ORDER BY term_type`);
                for (const row of termsRes.rows) {
                    await client.query(`INSERT INTO term_agreements (user_id, term_id)
             VALUES ($1, $2) ON CONFLICT (user_id, term_id) DO NOTHING`, [userId, row.id]);
                }
            }
            else {
                for (const tid of termIds) {
                    await client.query(`INSERT INTO term_agreements (user_id, term_id)
             VALUES ($1, $2) ON CONFLICT (user_id, term_id) DO NOTHING`, [userId, tid]);
                }
            }
            if (body.marketing_opt_in) {
                const m = await client.query(`SELECT id FROM terms WHERE term_type = 'marketing' AND status = 'published' LIMIT 1`);
                if (m.rows[0]) {
                    await client.query(`INSERT INTO term_agreements (user_id, term_id)
             VALUES ($1, $2) ON CONFLICT (user_id, term_id) DO NOTHING`, [userId, m.rows[0].id]);
                }
            }
